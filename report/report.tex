...
The initial diagram is a standard Agile loop, of which an example could be find in Figure~\ref{fig:agile-loop}. Its strength is rapid iteration; its weakness is that values may remain implicit unless they are deliberately introduced into sprint artefacts.
...
Figure~\ref{fig:gore-sleec-detail} shows how the Agile-VSD loop is extended with a requirements refinement branch. Conceptual Engineering clarifies values during sprint planning, GORE turns them into normative goals, and SLEEC rules make them precise enough to feed the sprint backlog.
...
The previous requirements branch is connected to CI/CD gates. Normative goals and SLEEC rules become policy-as-code checks, those checks block or approve pipeline stages, and their decisions are stored as audit evidence.
...
Runtime assurance is performed after deployment and focuses on whether the deployed system remains trustworthy under actual operating conditions. Figure~\ref{fig:mlops-assurance} separates this runtime layer from the design-time gates and shows the monitoring and control mechanisms that remain active after deployment. It includes:
