...
\begin{enumerate}
    \item \textbf{Agile based on Value Sensitive Design (VSD)} \cite[Ch.~13]{umbrello_agile_vsd}: values and stakeholders enter the backlog.
    \item \textbf{Conceptual Engineering (CE) with value hierarchy} \cite[Secs.~2--4]{veluwenkamp_ce}: ambiguous values are clarified before becoming requirements transforming them into concepts.
    \item \textbf{GORE + SLEEC} \cite[Secs.~3--4]{silva_gore_sleec}: values become normative goals and then formal, testable rules.
    \item \textbf{MLOps + Compliance-as-Code + Runtime Assurance} \cite[Sec.~Implementing Ethics in the Development Pipeline]{bml_ethsecdevops}, \cite[Secs.~3--4]{potharalanka_cac}, \cite[Secs.~4--5]{singh_cac}, \cite[Secs.~3--5]{lu_responsible_ai}, \cite[Secs.~5.2.3--5.2.5 and~9.2.3]{taid_whitepaper}: ethical and legal requirements are enforced through CI/CD gates, policy engines, monitoring, and human oversight.
\end{enumerate}
...
an explicit \textbf{RACI Matrix} after the definition of the Sprint Backlog and before
...
\begin{itemize}
    \item \textbf{Responsible}: the actor who performs the task or implements the control;
    \item \textbf{Accountable}: the actor who owns the final decision or approval;
    \item \textbf{Consulted}: the actors whose expertise is required, such as legal, ethics, domain, or security experts;
    \item \textbf{Informed}: the actors who must be kept aware of decisions, failures, exceptions, or residual risks.
\end{itemize}

In this way, responsibility is not left implicit in the technical pipeline. The RACI Matrix
connects requirements, implementation tasks, SLEEC-derived rules, CaC policies,
quality gates, audit evidence, and runtime assurance mechanisms to identifiable roles.
...
SLEEC rules translate normative goals into a form that is close to natural language but structured enough for automated analysis. In the cited SLEEC work, rules have the general form ''\texttt{when <condition> then <response> [within <time>] [unless <exception>]}'' \cite[Sec.~4]{silva_gore_sleec}. In the pipeline, SLEEC is the bridge between ethical language and executable control: it keeps the rule understandable for human review while making triggers, responses, deadlines, and exceptions precise enough for tests and monitors.
...
Shift-left compliance means that checks are executed as early as possible, ideally before code reaches production. CaC should be integrated at multiple points, as defined in Table~\ref{tab:cac-controls}.
